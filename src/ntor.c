...
enum cobfs4_return_code server_ntor(EVP_PKEY * restrict ephem_keypair,
    ...
    if (ecdh_derive(ephem_keypair, remote_pubkey, secret_input) != COBFS4_OK) {
        goto error;
    }
    if (ecdh_derive(shared->ntor, remote_pubkey, secret_input + COBFS4_PUBKEY_LEN) != COBFS4_OK) {
        goto error;
    }
    ...
    if (hmac_gen((const uint8_t *) t_key, strlen(t_key), secret_input, secret_len, out->key_seed) != COBFS4_OK) {
        goto error;
    }

    if (hmac_gen((const uint8_t *) t_verify, strlen(t_verify), secret_input, secret_len, verify) != COBFS4_OK) {
        goto error;
    }
    ...
    if (hmac_gen((const uint8_t *) t_mac, strlen(t_mac), auth_input, auth_len, out->auth_tag) != COBFS4_OK) {
        goto error;
    }
    return COBFS4_OK;
...
enum cobfs4_return_code client_ntor(EVP_PKEY * restrict ephem_keypair,
    ...
    if (ecdh_derive(ephem_keypair, shared->ntor, secret_input + COBFS4_PUBKEY_LEN) != COBFS4_OK) {
        goto error;
    }
